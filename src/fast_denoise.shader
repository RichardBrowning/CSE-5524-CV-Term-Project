...
float4 mainImage(VertData v_in) : TARGET
{
    ...
    float4 pre=previous_output.Sample(textureSampler,v_in.uv+normalFlow);
    float4 centerColor=image.Sample(textureSampler, v_in.uv);
    // return lerp(lerp(centerColor,pre,0.5),previous_image.Sample(textureSampler,v_in.uv+normalFlow),1/3);
